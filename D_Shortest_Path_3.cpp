...
void shortDistance(ll n, ll m, vl &arr, vpr adj[], vl &dist) {
  ...
  while (!pq.empty()) {
    ...
    pr temp_pq = pq.top();
    ll parent = temp_pq.second;
    ll curr_dist = temp_pq.first;
    pq.pop();
    if (dist[parent] < curr_dist) {
      continue;
    }
    for (auto it : adj[parent]) {
      ...
      if (curr_dist + val + arr[child - 1] < dist[child]) {
        dist[child] = curr_dist + val + arr[child - 1];
...
void solve() {
  ...
  vl dist(n + 1, 1e18);
  shortDistance(n, m, arr, adj, dist);
