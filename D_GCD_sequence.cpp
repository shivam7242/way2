...
void solve() {
  ...
  for (ll i = 1; i < n; i++) {
    store.push_back(__gcd(arr[i], arr[i - 1]));
  }
  ll ind = -1;
  for (ll i = 1; i < n - 1; i++) {
  ...
  for (ll i = 1; i < n - 1; i++) {
    store1.push_back(__gcd(arr1[i], arr1[i - 1]));
  }

  for (ll i = 1; i < n - 1; i++) {
    store2.push_back(__gcd(arr2[i], arr2[i - 1]));
  }

  for (ll i = 1; i < n - 1; i++) {
    store3.push_back(__gcd(arr3[i], arr3[i - 1]));
  }
  ll f1 = 0, f2 = 0, f3 = 0;
  for (ll i = 1; i < n - 2; i++) {
    if (store1[i] >= store1[i - 1]) {
    } else {
      f1 = 1;
    }
  }
