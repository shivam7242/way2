...
{
public:
  int fun(int ind, int n, vector<int> &arr, int &curr_reward)
  {
    if (ind == n)
    {
      return 0;
    ...
    int pick = 0;
    int not_pick = 0;
    if (arr[ind] > curr_reward)
    {
      curr_reward = curr_reward+arr[ind];
      pick = fun(ind + 1, n, arr, curr_reward);
    }
    else
    {
      not_pick = (ind + 1, n, arr, curr_reward);
    }
